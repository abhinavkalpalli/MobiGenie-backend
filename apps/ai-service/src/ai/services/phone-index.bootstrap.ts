...
export class PhoneIndexBootstrap implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap() {
    try {
      ...
      // Count only embeddings generated by the currently-configured model —
      // stale embeddings from a previous model (different dimensions) don't
      // count as "indexed" and get regenerated below.
      const indexedCount = await this.embeddingRepository['model']
        .countDocuments({ embeddingModel: EMBEDDING_MODEL })
        .exec();
