...
export class PhoneIndexBootstrap implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap() {
    try {
      ...
      const indexedCount = await this.embeddingRepository['model']
        .countDocuments()
        .exec();

      if (indexedCount >= totalPhones) {
        this.logger.log(
          `PhoneEmbedding already up-to-date (${indexedCount}/${totalPhones})`,
        );
        return;
      }

      this.logger.log(
        `Indexing phones: ${indexedCount} indexed, ${totalPhones} total`,
      );
